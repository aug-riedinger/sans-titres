...
scr.height/this.segmentY&&(this.strengthY=(this.Y-scr.height/this.segmentY)/(scr.height/this.segmentY));this.Y>scr.height-scr.height/this.segmentY&&(this.strengthY=1-(scr.height-this.Y)/(scr.height/this.segmentY))};Cursor.prototype.inTriangle=function(a,b,c){var d=c.X-a.X,e=c.Y-a.Y;c=b.X-a.X;var f=b.Y-a.Y,l=this.X-a.X,g=this.Y-a.Y;a=d*d+e*e;b=d*c+e*f;d=d*l+e*g;e=c*c+f*f;c=c*l+f*g;f=1/(a*e-b*b);l=(e*d-b*c)*f;a=(a*c-b*d)*f;return 0<=l&&0<=a&&1>l+a};
...
Room.prototype.init=function(a){this.name=a.name||"Room "+this.id;this.position=a.position;this.map=a.map;this.adj=a.adj||[];this.color=a.color;this.artsConstr=a.arts||[];this.soundsConstr=a.sounds||[];this.readMap();this.startFloor||(this.startFloor=this.floors[parseInt(this.floors.length/2,10)]);this.makeSounds();rooms[rooms.length]=this;return this};
Room.prototype.getElementsToRender=function(){var a,b,c,d=[],e=0;for(a=0;a<this.sounds.length;a++)this.sounds[a].adjustVolume(camera.x.value,camera.z.value);for(b in this.tops)if(this.tops.hasOwnProperty(b))for(c in this.tops[b])this.tops[b].hasOwnProperty(c)&&(d[e]=getEdges(this.tops[b][c],"top"),e+=1);for(b in this.bottoms)if(this.bottoms.hasOwnProperty(b))for(c in this.bottoms[b])this.bottoms[b].hasOwnProperty(c)&&(d[e]=getEdges(this.bottoms[b][c],"bottom"),e+=1);for(b in this.lefts)if(this.lefts.hasOwnProperty(b))for(c in this.lefts[b])this.lefts[b].hasOwnProperty(c)&&
(d[e]=getEdges(this.lefts[b][c],"left"),e+=1);for(b in this.rights)if(this.rights.hasOwnProperty(b))for(c in this.rights[b])this.rights[b].hasOwnProperty(c)&&(d[e]=getEdges(this.rights[b][c],"right"),e+=1);for(a=0;a<this.arts.length;a++)b=this.arts[a],b.projection(),b.visible&&(d[e]={type:"art",distance:0.5*this.arts[a].distance,art:this.arts[a]},e+=1);return d};Room.prototype.isTop=function(a){return"#"===a||("-"===a||"+"===a)||!1};
...
Room.prototype.readMap=function(){var a,b,c,d,e,f,l,g,h,n,q,p,m;for(a=0;a<this.map.length;a++){d=this.map.length-(a+1);for(b=0;b<this.map[a].length;b+=2)if(c=b/2,e=this.map[a][b],l=this.map[a][b+1],f=l.replace(/^[^a-zA-Z0-9]$/,""),this.isInside(e)){h=this.putFaceToWall(this.tops,this.isTop(e),d,faceMaker.top(this,c,d));n=this.putFaceToWall(this.bottoms,this.isBottom(e),d,faceMaker.bottom(this,c,d));q=this.putFaceToWall(this.lefts,this.isLeft(e),c,faceMaker.left(this,c,d));p=this.putFaceToWall(this.rights,
this.isRight(e),c,faceMaker.right(this,c,d));g=m=void 0;if(""!==f){g=this.getArtConstr(f);if(this.isTop(g.side||e))m=faceMaker.art(this,h,g),this.arts.push(m);if(this.isBottom(g.side||e))m=faceMaker.art(this,n,g),this.arts.push(m);if(this.isLeft(g.side||e))m=faceMaker.art(this,q,g),this.arts.push(m);if(this.isRight(g.side||e))m=faceMaker.art(this,p,g),this.arts.push(m)}c=faceMaker.floor(this,c,d,[h,n,q,p],m);"@"===l&&(this.startFloor=c);g&&"monolythe"===g.type&&(m=new Monolythe(c,g),c.f.select=!1,
this.arts=this.arts.concat(m.faces));this.floors.push(c)}}};Room.prototype.putFaceToWall=function(a,b,c,d){if(b)return a[c]||(a[c]=[]),0===a[c].length&&a[c].push([]),a[c][a[c].length-1].push(d),d;a[c]&&(0===a[c].length||0!==a[c][a[c].length-1].length)&&a[c].push([]);return null};Room.prototype.makeSounds=function(){var a,b;for(a=0;a<this.soundsConstr.length;a++)b=this.soundsConstr[a],this.sounds.push(new Sound(this,b))};
Room.prototype.enter=function(){0<this.sounds.length&&("none"===$("#volume").css("display")&&$("#volume").fadeIn(1E3),$("#volume").click($.proxy(function(){var a;for(a=0;a<this.sounds.length;a++)this.sounds[a].muted?(this.sounds[a].audio.volume=this.sounds[a].formerVolume,this.sounds[a].adjustVolume(),this.sounds[a].muted=!1,$("#volume").removeClass("muted")):(this.sounds[a].formerVolume=this.sounds[a].audio.volume,this.sounds[a].audio.volume=0,this.sounds[a].muted=!0,$("#volume").addClass("muted"))},
this)));for(i=0;i<this.sounds.length;i++)this.sounds[i].autoPlay&&(this.sounds[i].adjustVolume(this.startFloor.f.x,this.startFloor.f.z),this.sounds[i].audio.play())};Room.prototype.exit=function(){var a;$("#volume").fadeOut(1E3);for(a=0;a<this.sounds.length;a++)this.sounds[a].audio.pause()};var renderer={renderAll:function(){this.renderFloor();this.renderAiming();this.renderRooms()},renderFloor:function(){var a=camera.trig.sinX*(100*params.unit+camera.focalLength),b=Math.abs(camera.focalLength/(camera.trig.cosX*(100*params.unit+camera.focalLength)-camera.focalLength+camera.focalLength)*camera.zoom.value)||1E4,a=parseInt(0.5*scr.height+a*b,10);a<scr.height&&(scr.ctx.beginPath(),scr.ctx.rect(0,a,scr.width,scr.height-a),scr.ctx.fillStyle=colors.floor,scr.ctx.fill())},renderAiming:function(){cursor.aimedFace&&
...
(scr.ctx.strokeStyle=colors.aimedFloor,scr.ctx.stroke()))},renderRooms:function(){var a,b=[],c,d=0;for(a=0;a<rooms.length;a++){c=rooms[a].getElementsToRender();for(j=0;j<c.length;j++)b[d]=c[j],d+=1}renderer.renderElementsToRender(b)},renderElementsToRender:function(a){a=insertSortDistance(a);for(i=0;i<a.length;i++)"art"!==a[i].type&&renderer.facesMerged(a[i]),"art"===a[i].type&&(face=a[i].art,face.render())},facesMerged:function(a){var b,c;b=a.points;a=colors[a.type];if(2<b.length){scr.ctx.beginPath();
...
CanvasEl.Image.prototype.render=function(a,b,c,d,e,f){void 0===f&&(f=!0);var l=[a,b,c,d];if(this.isLoading)this.loading();else{var g=this.firstPoint;do{var h=a.X+g.ny*(d.X-a.X),n=a.Y+g.ny*(d.Y-a.Y);g.px=h+g.nx*(b.X+g.ny*(c.X-b.X)-h);g.py=n+g.nx*(b.Y+g.ny*(c.Y-b.Y)-n);g=g.next}while(g);d=this.canvas.width;g=this.canvas.height;h=this.firstTriangle;do{a=h.p0;b=h.p1;c=h.p2;var n=(a.px+b.px+c.px)/3,q=(a.py+b.py+c.py)/3,p=!0;if(0>n||n>d||0>q||q>g)if(0>Math.max(a.px,b.px,c.px)||Math.min(a.px,b.px,c.px)>
...
a.px)/h.d,(b.ty*c.py+a.ty*(b.py-c.py)-c.ty*b.py-h.pmy*a.py)/h.d,(a.tx*(c.px-b.px)-b.tx*c.px+c.tx*b.px+h.pmx*a.px)/h.d,-(b.tx*c.py+a.tx*(b.py-c.py)-c.tx*b.py-h.pmx*a.py)/h.d,(a.tx*(c.ty*b.px-b.ty*c.px)+a.ty*(b.tx*c.px-c.tx*b.px)+h.pxy*a.px)/h.d,(a.tx*(c.ty*b.py-b.ty*c.py)+a.ty*(b.tx*c.py-c.tx*b.py)+h.pxy*a.py)/h.d);this.ctx.drawImage(this.texture,0,0);this.ctx.restore()}h=h.next}while(h);if(f){this.ctx.beginPath();for(a=0;a<l.length;a++)this.ctx.lineTo(l[a].X,l[a].Y);this.ctx.closePath();this.ctx.lineWidth=
...
$("#artClearView").one("click",function(){remHtml()})},remHtml=function(){$("#artClearView").fadeOut(1E3,function(){$("#artClearView").empty()})},getEdges=function(a,b){if(0===a.length)return{distance:-99999,points:[]};var c,d,e,f,l,g=0,h=0;for(c=0;c<a.length;c++)if(d=a[c],d.projection(),d.visible){!e&&(!f&&!l)&&(e=f=a[c],l=a[c].distance);h+=1;g+=d.distance;if("left"===b||"right"===b)d.f.z<e.f.z&&(e=d),d.f.z>f.f.z&&(f=d);if("top"===b||"bottom"===b)d.f.x<e.f.x&&(e=d),d.f.x>f.f.x&&(f=d)}if(!e&&!f&&
!l)return{distance:-99999,points:[]};if("top"===b||"left"===b)return{type:b,distance:g/h,points:[e.p3,e.p0,f.p1,f.p2]};if("bottom"===b||"right"===b)return{type:b,distance:g/h,points:[e.p1,e.p2,f.p3,f.p0]}},getEdges2=function(a,b){var c,d=[],e=0,f,l,g,h,n,q,p,m,r,s,t,u=0;for(c=n=l=g=h=0;c<a.length;c++)f=a[c],f.projection(),f.visible&&(l+=f.f.x,g+=f.f.y,h+=f.f.z,n++,f.p0.behind||(d[e]=f.p0,e+=1),f.p1.behind||(d[e]=f.p1,e+=1),f.p2.behind||(d[e]=f.p2,e+=1),f.p3.behind||(d[e]=f.p3,e+=1));l=l/n||0;g=g/
n||0;h=h/n||0;d=remMany(d,equalsCoord,4);if(0<d.length){e=d[0];for(c=0;c<d.length;c++)u<d[c].distance&&(u=d[c].distance),f=e.x-l,n=e.y-g,q=e.z-h,p=d[c].x-l,m=d[c].y-g,r=d[c].z-h,"x"===b&&(s=n*m+q*r,t=n*r-q*m),"y"===b&&(s=f*p+q*r,t=q*p-f*r),"z"===b&&(s=f*p+n*m,t=f*m-n*p),f=Math.atan(t/s)||0,d[c].theta=0<=s?f:f+Math.PI,d[c].cosTheta=s,d[c].sinTheta=t;d=insertSortTheta(d)}return{distance:u,points:d}},remMany=function(a,b,c){var d,e,f=[],l=[],g;for(d=0;d<a.length;d++){g=!1;for(e=0;e<l.length;e++)b(a[d],
l[e])&&(f[e]+=1,g=!0);g||(l.push(a[d]),f.push(1))}for(d=0;d<l.length;d++)f[d]>=c&&(l.splice(d,1),f.splice(d,1),d--);return l},equalsCoord=function(a,b){return a.x===b.x&&a.y===b.y&&a.z===b.z};var startCpt=function resetCpt(){!SLOW&&50>cpt&&(console.log(cpt),console.log("Slow Mode"),SLOW=!0);SLOW&&100<cpt&&(console.log(cpt),console.log("Fast Mode"),SLOW=!1);cpt=0;setTimeout(resetCpt,5E3)},enteredRoom=function(a){var b,c,d;rooms[0].exit();for(k=0;k<rooms.length;k++)if(rooms[k].id===a){a=rooms[k];rooms[k]=rooms[0];rooms[0]=a;d=rooms[0];for(a=0;a<d.adj.length;a++){c=null;for(b=1;b<rooms.length;b++)if(rooms[b].id===d.adj[a]){c=rooms[b];break}c||new Room(d.adj[a])}for(b=1;b<rooms.length;b++){c=
...
a.art){var b;for(b=0;b<rooms[0].floors.length;b++)rooms[0].floors[b].f.art&&rooms[0].floors[b].f.art.f.artId===a.art&&camera.targetToFace(rooms[0].floors[b])}requestAnimFrame(run);$(scr.canvas).fadeIn(3E3,function(){setTimeout(remHtml,1E3)})});startCpt()},run=function(){MENU||requestAnimFrame(run);scr.ctx.clearRect(0,0,scr.width,scr.height);camera.move();renderer.renderAll();cpt++};init();var MENU=!0;$("#visite").click(function(){$("#menu").fadeOut(1E3);$("#screen").fadeIn(1E3);MENU=!1;run()});var Sound=function(a,b){this.audio=new Audio;this.audio.src=b.src;this.autoPlay=b.play;this.muted=!1;b.position&&(this.position={x:(a.position.x+b.position.x)*params.unit,z:(a.position.z+b.position.z)*params.unit})};Sound.prototype.adjustVolume=function(a,b){var c;this.position?(c=Math.sqrt((a-this.position.x)*(a-this.position.x)+(b-params.focalLength-this.position.z)*(b-params.focalLength-this.position.z)),c=params.unit/c,1<c&&(c=1),0>c&&(c=0)):c=0.8;this.audio.volume=c};
